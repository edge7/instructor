...
from .types import TemplateType, TemplateKwargsType, LLMError, TemplateError
...
class AsyncEngine:
    ...
    def __init__(self, client: Optional[Any] = None, max_concurrency: int = 32):
        ...
        self.max_concurrency = int(os.getenv("PROMPTFRAME_MAX_CONCURRENCY", max_concurrency))
    ...
    def _get_template(self, template_str: str) -> Template:
        ...
        if template_str not in self._template_cache:
            self._template_cache[template_str] = self.jinja_env.from_string(template_str)
