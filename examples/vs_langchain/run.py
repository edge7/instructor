...
class LangChainBenchmark:
    ...
    def get_token_usage(self, text: str) -> int:
        ...
        format_instructions = self.parser.get_format_instructions()
        full_prompt = self.prompt.format(text=text, format_instructions=format_instructions)
...
def benchmark_performance(num_iterations: int = 10) -> Dict[str, Any]:
    ...
    results = {
        "instructor": {"available": instructor_bench.available, "time": 0.0, "results": []},
        "langchain": {"available": langchain_bench.available, "time": 0.0, "results": []}
    }
    ...
    if results["instructor"]["available"] and results["langchain"]["available"]:
        ...
        if instructor_time > 0 and langchain_time > 0:
            ...
            print(f"  Difference: {speedup:+.1f}% ({'faster' if speedup < 0 else 'slower'} than Instructor)")
            
            # Blog post claims LangChain is 17% slower
            blog_claim = 17.0
            print(f"  Blog claim: LangChain is {blog_claim}% slower")
            print(f"  Actual:     LangChain is {-speedup:.1f}% {'faster' if speedup < 0 else 'slower'}")
...
def benchmark_tokens() -> Dict[str, Any]:
    ...
    if langchain_bench.available:
        ...
        if results["instructor"] > 0:
            ...
            print(f"  Savings:    {savings:.1f}% (Instructor uses {savings:.1f}% fewer tokens)")
...
'''
    ...
    instructor_lines = len([l for l in instructor_code.strip().split('\n') if l.strip() and not l.strip().startswith('#')])
    langchain_lines = len([l for l in langchain_code.strip().split('\n') if l.strip() and not l.strip().startswith('#')])
    
    print(f"Lines of code (excluding comments):")
    print(f"  Instructor: {instructor_lines} lines")
    print(f"  LangChain:  {langchain_lines} lines")
    print(f"  Difference: {langchain_lines - instructor_lines} more lines for LangChain")
...
def run_all_benchmarks():
    ...
    if perf["instructor"]["available"] and perf["langchain"]["available"]:
        ...
        if inst_time > 0 and lang_time > 0:
            ...
            print(f"⚡ Performance: LangChain is {-speedup:.1f}% {'faster' if speedup < 0 else 'slower'}")
    ...
    if isinstance(tokens["instructor"], int) and isinstance(tokens["langchain"], int):
        if tokens["langchain"] > 0:
            savings = (tokens["langchain"] - tokens["instructor"]) / tokens["langchain"] * 100
