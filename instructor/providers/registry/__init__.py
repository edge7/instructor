...
class ProviderRegistry:
    ...
    def register(cls, provider_class: type[BaseProvider]) -> type[BaseProvider]:
        ...
        if not hasattr(provider_class, "name"):
            raise ValueError(f"Provider class {provider_class.__name__} must define a 'name' attribute")
            
        if not hasattr(provider_class, "supported_modes"):
            raise ValueError(f"Provider class {provider_class.__name__} must define 'supported_modes'")
    ...
    @classmethod
    def validate_provider(cls, provider: BaseProvider, required_modes: set[Mode]) -> None:
